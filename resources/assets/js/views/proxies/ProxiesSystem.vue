<template>
    <div>
        <page-header icon="fa-cubes" title="PROXY для работы системы">
        ...
        <div v-if="proxies" class="container-fluid">
            <div class="m-b-lg">
                <button class="btn btn-success" @click="showModal('modal-add-proxy-system')">
            ...
            <div class="btn-group">
                <a href="#" class="btn btn-primary">Перепроверить</a>
            </div>
            <div class="btn-group">
        ...
        <add-proxy-system-modal @add-proxies="addProxies"></add-proxy-system-modal>
    </div>
</template>

<script>
  import Table from './../../mixins/table';
  import AddProxySystemModal from './AddProxySystemModal.vue';

  export default {
    mixins: [Table],
    components: { AddProxySystemModal },
    mounted() {
    ...
    methods: {
      fetchProxies () {
        this.loading = true;
        axios.get('/api/proxies', { params: { type: 'system' } })
          .then((response) => {
